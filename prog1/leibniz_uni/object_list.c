...
void print_list(Node* list) {
    if (list == NULL) {
        ...
        Object* o = list->value;
        String s =  o->class->to_string(o);
        printf("[%s", s);
        free(s);
        for (Node* n = list->next; n != NULL; n = n->next) {
            o = n->value;
            s = o->class->to_string(o);
            printf(", %s", s);
            free(s);
        }
        printf("]");
...
void free_list(Node* list, bool free_element) {
    ...
    for (Node* node = list; node != NULL; node = node_next) {
        node_next = node->next;
        Object* o = node->value;
        if (free_element) o->class->free(o);
        free(node);
    }
}
...
bool contains_list(Node* list, void* element, bool identity_based) {
    if (identity_based) { // identity-based equality
        ...
        for (Node* node = list; node != NULL; node = node->next) {
            Object* o = node->value;
            if (o->class->equal(o, element)) { // compare content
                return true;
            }
        }
    }
...
Node* copy_list(Node* list, bool copy_element) {
    if (list == NULL) {
        ...
        if (copy_element) { // just reassign, do not copy elements
            Object* o = list->value;
            Node* result = new_node(o->class->copy(o), NULL);
            for (Node* n = result; list->next != NULL; n = n->next) {
                list = list->next;
                o = list->value;
                n->next = new_node(o->class->copy(o), NULL);
            }
            return result;
        } else { // copy elements
