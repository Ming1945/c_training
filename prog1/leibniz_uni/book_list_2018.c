...
Book* new_book(String authors, String title, int year) {
    ...
    b->authors = s_copy(authors); //why copy? why not just like b->authors = authors; bcs String is pointer?
...
int compare_books(void* x, void* y) {
    ...
    Book* b = (Book*)y;
    if (a == b) return 0;
    if (a == NULL) return -1; // what it means?
    if (b == NULL) return 1;
